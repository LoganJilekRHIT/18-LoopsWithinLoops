...
def run_test_sum_numbers():
    ...
    expected = 17
    answer = sum_numbers(([1, 5], [1], [4, 3, 3]))
    print('Expected and actual are:', expected, answer)
...
def sum_numbers(seq_seq):
    ...
    for k in range(len(seq_seq)):
        sub_guy = seq_seq[k]
        for j in range(len(sub_guy)):
            sub_total = sub_total + sub_guy[j]
        total = total + sub_total                   # WHY DON'T I NEED THIS??????
    return sub_total
